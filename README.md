...
use with [Soele](https://github.com/Floral-Overcast/SoeleSilico) (a self-hosted agent switchboard, which
can drive this lifecycle from its UI) but useful standalone.

## The idea

An agent session should live in a room you can afford to lose, separately from the session that helps you
install drivers and fixes your microphone settings. That way it only reads memory relevant to the project at hand, there is no
physical way it can waste time and tokens considering your pc setup or whatever. The template
...
Soele's daemon drives this lifecycle when `HUB\_CTS=1` and `HUB\_CT\_BASE\_IP`
